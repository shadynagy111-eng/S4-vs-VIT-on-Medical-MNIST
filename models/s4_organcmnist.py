# -*- coding: utf-8 -*-
"""s4-organcmnist.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1BGW3PlazaN0rBLjRYLO5DsJjatyWKkka
"""
...
validation_dataset = OrganCMNIST(split="val", download=True)
validation_dataset
...
train_data_transformed = apply_transforms(train_dataset.imgs, my_transforms)
val_data_transformed = apply_transforms(validation_dataset.imgs, my_transforms)
test_data_transformed=apply_transforms(test_dataset.imgs,my_transforms)
...
# 1. Change 'checkpoint_flag' back to 'checkpoint'
def eval(epoch, val_loader, checkpoint=False):
    global best_acc
    model.eval()
    ...
    # 2. Use the 'checkpoint' variable here
    if checkpoint:
        if acc > best_acc:
            state = {
    ...
    # 3. Ensure both are returned for your history loop
    return avg_loss, acc
...
for epoch in pbar:
    ...
    train()
    val_loss,val_acc = eval(epoch, val_loader, checkpoint=True)
...
# --- ROBUST EXECUTION ---
possible_paths = ['./checkpoint/ckpt.pth', 'ckpt.pth', './ckpt.pth']
...
if checkpoint_path:
    targets, preds, cm = evaluate_and_plot_cm(model, test_loader, checkpoint_path, device)
else:
    print("❌ Error: Checkpoint not found. Please run the training loop first!")
...
summary(model, input_size=(1,784,1))
...
for epoch in range(num_epochs):
    ...
    t_loss, t_acc = train()
    v_loss, v_acc = eval(epoch, val_loader, checkpoint=True)
